class MapCell {  
  ...
  public MapCell(int terrain, int x, int y) {
    ...
    switch (this.terrain) {
      ...
      case 12: defense = 3; buildingOwner = -1;	buildingHealth = 20; break;    																												//city, is always neutral on creation
      case 13: defense = 2; buildingOwner = 0; 	buildingHealth = 20; players[0].ownedBuildings++; isFactory = true; break;    				//factory, player 1
      case 14: defense = 3; buildingOwner = 0;  buildingHealth = 20; players[0].ownedBuildings++; break;    													//HQ, player player 1
      case 15: defense = 2; buildingOwner = 1; 	buildingHealth = 20; players[1].ownedBuildings++; isFactory = true; break;   					//factory, player 2
      case 16: defense = 3; buildingOwner = 1; 	buildingHealth = 20; players[1].ownedBuildings++; break;    													//HQ, player 2
      default: cost = new int[] {1,1,1,1}; defense = 1; break;
    }
    if (this.terrain >= 12 && this.terrain <= 16) {
      cost = new int[] {1,1,1,1};
      isBuilding = true;
    }
...
class MapGrid {
    ...
    for (int b = 0; b < mapBytes.length; b++) {				//check to make sure the map data is properly encoded (width and height bytes match # of tiles)...		
    	if(mapBytes[b] == -1 && b-2 != wMap*hMap) {			//by seeing if the index of mapBytes b-2 (to account for the width and height bytes), is equal to the width times height
    		...
    		//because if I let any random user submitted data go in, it could be bad. Trust me. I'm a C++ programmer. user input kills all.
    	}
    }
		print('\n');
